...
ls "$BACKUP_DIR" | while read line; do
  ...
  if [[ ! -d "${backupdir}/${line}" ]]; then
    dest_path=sync_backups/${foldername}/FILES/${filename}_${starttime}.tar.gz.gpg
  else
    dest_path=sync_backups/${foldername}/${filename}_${starttime}.tar.gz.gpg
  fi

  echo `date +%Y%m%d\ %H:%M:%S` : Compression will begin : ${filename}
  # Compression and encryption of the file/folder
  tar -P -czf - "${backupdir}/${line}" | gpg --batch --yes --passphrase ${passphrase} -ac -o "/home/pierre/hubic/tmp/${filename}.tar.gz.gpg"
  ...
  actualsize=$(du -k "/home/pierre/hubic/tmp/${filename}.tar.gz.gpg" | cut -f 1)
  ...
  $hubicpy --swift -- upload --use-slo --segment-size ${segmentSize} --object-threads 40 --segment-threads 20 --object-name "${dest_path}" default /home/pierre/hubic/tmp/${filename}.tar.gz.gpg
  ...
  if [ $? -eq 0 ]; then
    ...
    $hubicpy --swift -- list default_segments | grep "sync_backups/$(basename ${backupdir})/${filename}.tar.gz.gpg" | xargs -I {} sh -c "${hubicpy} --swift delete {}"
    exit 1
    #TODO Do not exit the script but continu and warn of the error at the end of the script
  fi

rm -f /home/pierre/hubic/tmp/${filename}.tar.gz.gpg
